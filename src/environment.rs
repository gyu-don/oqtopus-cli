...
use crate::metadata::{INVALID_UTF8, metadata_get, migrate_metadata_keys};
...
impl ValidatedMetadata {
    fn require(&self, key: &str) -> Result<String, String> {
        metadata_get(&self.contents, key)
            .map(str::to_owned)
    ...
    fn require_compat(&self, key: &str, legacy_key: &str) -> Result<String, String> {
        metadata_get(&self.contents, key)
            .or_else(|| metadata_get(&self.contents, legacy_key))
            .map(str::to_owned)
            .ok_or_else(|| missing(key))
    }
...
fn validated_metadata(template_name: &str) -> Result<ValidatedMetadata, String> {
    ...
    // Retain the legacy missing-key diagnostic for ordinary read failures, while rejecting an
    // unsupported byte encoding explicitly instead of changing its contents through lossy decode.
    ...
    let template = metadata_get(&contents, "template").ok_or_else(|| missing("template"))?;
    ...
    let root = metadata_get(&contents, "environment_root")
        .or_else(|| metadata_get(&contents, "env_root"))
